# ------------------------------------------------------------------------------
# Simulation setting with n = 250, rho = 0.3 and var = 5, on MCMC-based models
# ------------------------------------------------------------------------------
...
export.results.mcmc(n, rho, sigma2 = sigma2, reps)

# Load the results -------------------------------------------------------------

path <- paste0(subfolder.path(n, rho, sigma2 = sigma2, reps, bbssl = F), "/")
paste0(path, current.set(n, rho, sigma2 = sigma2, reps), ".Rdata") |> load()

# Print the results ------------------------------------------------------------

# n = 250, rho = 0.3, sigma2 = 5
cat("n =", n, "rho =", rho, "var = ", 5)

out.mcmc$MSE$MSE.bnp |> mean(x = _, na.rm = T) |> round(x = _, 3) 
